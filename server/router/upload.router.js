...
router.get('/files/:filename', (req, res) => {
    ...
    const filePath = path.join(path.resolve(__dirname, '..'), 'uploads', "files", filename);
    res.sendFile(filePath);
});

router.get('/generate/:filename', (req, res) => {
    const filename = req.params.filename;
    const filePath = path.join(path.resolve(__dirname, '..'), 'uploads', "generate", filename);
    res.sendFile(filePath);
});
router.get('/attendance/:filename', (req, res) => {
    const filename = req.params.filename;
    const filePath = path.join(path.resolve(__dirname, '..'), 'uploads', "attendance", filename);
    res.sendFile(filePath);
});
